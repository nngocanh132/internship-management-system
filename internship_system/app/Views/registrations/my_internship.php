<?php // View: registrations/my_internship — nhận $reg từ RegistrationController ?>
<?php include BASE_PATH_FS . 'includes/header.php'; ?>
<div class="ph fu"><div><h4><i class="bi bi-briefcase-fill me-2"></i>Thực tập của tôi</h4></div></div>
<?php showFlash(); ?>
...
<div class="card text-center py-5 fu1"><div class="card-body">
  ...
  <a href="<?=BASE_PATH?>/internships/browse.php" class="btn btn-primary"><i class="bi bi-search me-1"></i>Tìm việc thực tập</a>
</div></div>
<?php else:
...
<div class="row g-4">
  <div class="col-md-4">
    ...
    <div class="card mb-3 fu1" style="border-left:4px solid #4ab8c4"><div class="card-body">
      ...
      <?php if($reg['department']??''): ?><div class="small text-muted mb-1"><i class="bi bi-building me-1"></i><?=htmlspecialchars($reg['department'])?></div><?php endif; ?>
      <?php if($reg['l_email']??''): ?><div class="small mb-1"><i class="bi bi-envelope me-2" style="color:#3a8a96"></i><a href="mailto:<?=htmlspecialchars($reg['l_email'])?>" style="color:#3a8a96"><?=htmlspecialchars($reg['l_email'])?></a></div><?php endif; ?>
      <?php if($reg['l_phone']??''): ?><div class="small mb-2"><i class="bi bi-telephone me-2" style="color:#3a8a96"></i><a href="tel:<?=htmlspecialchars($reg['l_phone'])?>" style="color:#3a8a96"><?=htmlspecialchars($reg['l_phone'])?></a></div><?php endif; ?>
      <?php if($reg['l_email']??''): ?>
      <a href="<?=BASE_PATH?>/messages/lecturer_chat.php" class="btn btn-sm w-100" style="background:rgba(74,138,150,.12);color:#3a8a96;border:1px solid rgba(74,138,150,.3)">
        <i class="bi bi-person-workspace me-1"></i>Nhắn tin GVHD
      </a>
      <?php endif; ?>
    </div></div>
    <?php else: ?>
    ...
    <div class="card mb-3 fu2"><div class="card-body">
      ...
      <?php if($reg['address']??''): ?><div class="small mb-1"><i class="bi bi-geo-alt me-2 text-muted"></i><?=htmlspecialchars($reg['address'])?></div><?php endif; ?>
      ...
      <a href="<?=BASE_PATH?>/messages/chat.php?company_id=<?=$reg['company_id']?>" class="btn btn-primary btn-sm w-100">
    ...
    <div class="card fu3"><div class="card-body">
      ...
      <div class="d-grid gap-2">
        <a href="<?=BASE_PATH?>/reports/submit.php" class="btn btn-primary btn-sm">
  ...
  <div class="col-md-8">
    <!-- Evaluation -->
    <?php if($reg['overall_score']??0): ?>
    <div class="card mb-3 fu1" style="border-left:4px solid <?=$reg['overall_score']>=7?'#2d6a40':($reg['overall_score']>=5?'var(--ds)':'#9a3030')?>"><div class="card-body">
      <div class="d-flex justify-content-between align-items-center mb-3">
        <h6 class="fw7 mb-0"><i class="bi bi-star-fill me-2" style="color:#a07040"></i>Đánh giá từ doanh nghiệp</h6>
        <div><span style="font-size:2.5rem;font-weight:800;color:<?=$reg['overall_score']>=7?'#2d6a40':($reg['overall_score']>=5?'var(--ds)':'#9a3030')?>;line-height:1"><?=$reg['overall_score']?></span><span class="text-muted small">/10</span></div>
      </div>
      <div class="row g-2 mb-3">
        ...
        <div class="col-6">
          ...
          <div class="d-flex align-items-center gap-2">
            <div style="flex:1;height:7px;background:rgba(164,195,162,.22);border-radius:4px;overflow:hidden"><div style="height:7px;background:linear-gradient(90deg,var(--ds),var(--sg));border-radius:4px;width:<?=($reg[$k]/10*100)?>%"></div></div>
            <span class="fw7 small"><?=$reg[$k]?></span>
          </div>
        </div>
        <?php endforeach; ?>
      </div>
      <?php if($reg['ev_comment']??''): ?><div style="background:rgba(164,195,162,.08);border-radius:9px;padding:12px;font-size:.85rem"><strong>Nhận xét:</strong> <?=nl2br(htmlspecialchars($reg['ev_comment']))?></div><?php endif; ?>
    </div></div>
    <?php else: ?>
    <div class="card mb-3 fu1" style="border:1.5px dashed rgba(164,195,162,.4)"><div class="card-body text-center py-3">
    ...
    <!-- Report status -->
    <?php if($reg['report_file']??$reg['rep_status']??''): ?>
    <div class="card fu2" style="border-left:4px solid <?=$reg['rep_status']==='approved'?'#2d6a40':($reg['rep_status']==='rejected'?'#9a3030':'var(--ds)')?>"><div class="card-body">
      <div class="d-flex justify-content-between align-items-start">
        <h6 class="fw7 mb-2"><i class="bi bi-file-earmark-text me-2"></i>Báo cáo thực tập</h6>
        <?php $rs_map=['pending'=>['⏳ Chờ duyệt','rgba(196,154,108,.12)','#a07040'],'approved'=>['✅ Đã duyệt','rgba(74,158,106,.12)','#2d6a40'],'rejected'=>['❌ Cần sửa lại','rgba(192,96,80,.12)','#9a3030']];[$rl,$rb,$rc]=$rs_map[$reg['rep_status']]??['—','rgba(160,160,160,.1)','#5a5a5a']; ?>
        <span class="badge" style="background:<?=$rb?>;color:<?=$rc?>"><?=$rl?></span>
      </div>
      <?php if($reg['rep_submitted']??''): ?><div class="small text-muted mb-2">Nộp lúc: <?=date('d/m/Y H:i',strtotime($reg['rep_submitted']))?></div><?php endif; ?>
      <?php if($reg['lecturer_comment']??''): ?><div style="background:rgba(164,195,162,.08);border-radius:9px;padding:10px 12px;font-size:.82rem"><strong>Nhận xét GVHD:</strong> <?=nl2br(htmlspecialchars($reg['lecturer_comment']))?></div><?php endif; ?>
      <div class="d-flex gap-2 mt-2">
        ...
        <?php if(($reg['rep_status']??'')==='rejected'): ?><a href="<?=BASE_PATH?>/reports/submit.php" class="btn btn-primary btn-sm"><i class="bi bi-arrow-repeat me-1"></i>Nộp lại</a><?php endif; ?>
      </div>
    </div></div>
    <?php else: ?>
    <div class="card fu3" style="border:1.5px dashed rgba(164,195,162,.4)"><div class="card-body text-center py-3">
      ...
      <a href="<?=BASE_PATH?>/reports/submit.php" class="btn btn-primary btn-sm mt-2"><i class="bi bi-upload me-1"></i>Nộp ngay</a>
    </div></div>
    <?php endif; ?>
  </div>
</div>
