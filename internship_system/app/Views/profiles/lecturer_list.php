<?php // View: profiles/lecturer_list — nhận $lecturers ?>
...
<div class="ph fu">
  ...
  <a href="<?=BASE_PATH?>/users/create_lecturer.php" class="btn btn-primary"><i class="bi bi-person-plus me-1"></i>Thêm giảng viên</a>
</div>
<?php showFlash(); ?>
...
<div class="card text-center py-5 fu1"><div class="card-body">
  ...
  <a href="<?=BASE_PATH?>/users/create_lecturer.php" class="btn btn-primary mt-2">Thêm giảng viên</a>
</div></div>
<?php else: ?>
...
<div class="col-md-6 col-lg-4" style="animation:fadeUp .32s <?=$i*.04?>s ease both">
  <div class="card h-100" style="border:1.5px solid rgba(164,195,162,.2)"><div class="card-body">
    <div class="d-flex align-items-center gap-3 mb-3">
      <div class="av" style="width:46px;height:46px;font-size:1rem;background:linear-gradient(135deg,#4ab8c4,#2a95a2)"><?=strtoupper(mb_substr($l['full_name'],0,1))?></div>
      <div><div class="fw7"><?=htmlspecialchars($l['full_name'])?></div><div class="small text-muted"><?=htmlspecialchars($l['department']??'—')?></div></div>
    </div>
    <div class="small mb-1"><i class="bi bi-envelope me-2 text-muted"></i><?=htmlspecialchars($l['email'])?></div>
