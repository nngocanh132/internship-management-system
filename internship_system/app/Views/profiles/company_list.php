<?php // View: profiles/company_list — nhận $companies ?>
<?php include BASE_PATH_FS . 'includes/header.php'; ?>
<div class="ph fu"><div><h4><i class="bi bi-building-fill me-2"></i>Doanh nghiệp</h4><div class="ph-sub">Tổng: <?=count($companies)?></div></div></div>
<?php showFlash(); ?>
<?php if(empty($companies)): ?>
<div class="card text-center py-5 fu1"><div class="card-body"><i class="bi bi-building" style="font-size:3rem;color:var(--tl)"></i><h5 class="mt-3 fw7">Chưa có doanh nghiệp nào đăng ký</h5></div></div>
<?php else: ?>
...
<div class="col-md-6 col-lg-4" style="animation:fadeUp .32s <?=$i*.04?>s ease both">
  <div class="card h-100" style="border:1.5px solid rgba(164,195,162,.2)"><div class="card-body">
    <div class="d-flex align-items-center gap-3 mb-3">
      <img src="<?=$logo?>" style="width:48px;height:48px;border-radius:12px;object-fit:cover;flex-shrink:0">
      <div><div class="fw7"><?=htmlspecialchars($c['company_name'])?></div><div class="small text-muted"><?=htmlspecialchars($c['industry']??'—')?></div></div>
    </div>
    ...
    <div class="d-flex gap-2 mt-2 flex-wrap">
      ...
      <span class="badge ms-auto" style="<?=($c['is_profile_completed']??0)?'background:rgba(74,158,106,.12);color:#2d6a40':'background:rgba(196,154,108,.12);color:#a07040'?>"><?=($c['is_profile_completed']??0)?'✅ Đầy đủ':'⚠️ Chưa xong'?></span>
    </div>
