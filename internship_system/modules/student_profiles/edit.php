...
$stmt = $conn->prepare("SELECT * FROM company_evaluations WHERE eval_id=?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    foreach ([$attitude_score, $skill_score, $result_score] as $val) {
    ...
    if (empty($errors)) {
        ...
        $stmt = $conn->prepare(
            "UPDATE company_evaluations
             SET attitude_score=?, skill_score=?, result_score=?, total_score=?, comment=?
             WHERE eval_id=?"
        );
        $stmt->bind_param('ddddsi', $attitude_score, $skill_score, $result_score, $total, $comment, $id);
        if ($stmt->execute()) {
...
<div class="d-flex justify-content-between align-items-center mb-3">
    <h4><i class="bi bi-pencil-square me-2 text-warning"></i>Chỉnh sửa Đánh giá DN</h4>
...
<div class="card">
    <div class="card-body">
        <form method="POST">
            <div class="row g-3">
                <div class="col-md-4">
                    <label class="form-label fw-semibold">Thái độ (0–10)</label>
                    <input type="number" name="attitude_score" class="form-control"
                           min="0" max="10" step="0.5" value="<?= $eval['attitude_score'] ?>">
                </div>
                <div class="col-md-4">
                    <label class="form-label fw-semibold">Kỹ năng (0–10)</label>
                    <input type="number" name="skill_score" class="form-control"
                           min="0" max="10" step="0.5" value="<?= $eval['skill_score'] ?>">
                </div>
                <div class="col-md-4">
                    <label class="form-label fw-semibold">Kết quả (0–10)</label>
                    <input type="number" name="result_score" class="form-control"
                           min="0" max="10" step="0.5" value="<?= $eval['result_score'] ?>">
                </div>
                <div class="col-12">
