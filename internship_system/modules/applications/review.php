...
$stmt=$conn->prepare("SELECT a.*,
  sp.full_name,sp.student_code,sp.gpa,sp.major,sp.phone,sp.about_me,sp.avatar AS s_avatar,sp.linkedin_url,
  i.title,i.description,i.requirements,i.quantity,i.location,i.start_date,i.end_date,i.internship_id,
...
// Also get all other applications of this student
$other=$conn->prepare("SELECT a2.*,i2.title,cp2.company_name FROM applications a2 JOIN internships i2 ON a2.internship_id=i2.internship_id JOIN company_profiles cp2 ON i2.company_id=cp2.company_id WHERE a2.student_id=? AND a2.application_id!=? ORDER BY a2.applied_at DESC");
$other->bind_param('ii',$a['student_id'],$id); $other->execute();
$others=$other->get_result()->fetch_all(MYSQLI_ASSOC);
...
if($_SERVER['REQUEST_METHOD']==='POST'){
    ...
    if($action==='approve'){
        $u=$conn->prepare("UPDATE applications SET status='approved_admin',admin_note=? WHERE application_id=?");
        $u->bind_param('si',$note,$id); $u->execute();
        setFlash('success','✅ Đã duyệt — Chuyển sang công ty xét duyệt.');
    } elseif($action==='reject'){
        $u=$conn->prepare("UPDATE applications SET status='rejected_admin',admin_note=? WHERE application_id=?");
        $u->bind_param('si',$note,$id); $u->execute();
...
$av=$a['s_avatar']?UPLOAD_URL.'/'.$a['s_avatar']:'https://ui-avatars.com/api/?name='.urlencode($a['full_name']).'&background=5D7B6F&color=fff&size=100';
$clogo=$a['c_logo']?UPLOAD_URL.'/'.$a['c_logo']:'https://ui-avatars.com/api/?name='.urlencode($a['company_name']).'&background=A4C3A2&color=2A3F38&size=60';
?>
