...
if(isset($_GET['approve'])&&$cid){
    $id=(int)$_GET['approve'];
    $u=$conn->prepare("UPDATE applications SET status='approved_company' WHERE application_id=? AND internship_id IN (SELECT internship_id FROM internships WHERE company_id=?)");
...
$apps=$conn->prepare("SELECT a.*,sp.full_name,sp.student_code,sp.gpa,sp.major,sp.avatar AS s_avatar,sp.student_id,
  i.title,u.email
  FROM applications a
  ...
  <div class="card h-100" style="border:1.5px solid rgba(164,195,162,.25)">
    <div class="card-body">
      ...
      <div class="text-center mt-2">
        <a href="<?=BASE_PATH?>/modules/messages/chat.php?student_id=<?=$a['student_id']?>&app_id=<?=$a['application_id']?>" class="btn btn-primary w-100 btn-sm">
