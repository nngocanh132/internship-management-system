...
$st->execute(); $apps=$st->get_result()->fetch_all(MYSQLI_ASSOC);

$counts=[];
foreach(['pending_admin','approved_admin','rejected_admin','approved_company','rejected_company','interview_passed','interview_failed','internship_active','internship_completed'] as $s)
    $counts[$s]=$conn->query("SELECT COUNT(*) c FROM applications WHERE status='$s'")->fetch_assoc()['c'];
?>
