...
if (isset($_GET['approve']) && $cid) {
    $id  = (int)$_GET['approve'];
    $u   = $conn->prepare("UPDATE applications SET status='approved_company' WHERE application_id=? AND internship_id IN (SELECT internship_id FROM internships WHERE company_id=?) AND status='approved_admin'");
    if ($u) { $u->bind_param('ii',$id,$cid); $u->execute(); }
    setFlash('success','✅ Đã chấp nhận! Nhắn tin để hẹn lịch phỏng vấn.');
    redirect('company_candidates.php'.($job_filter?"?job=$job_filter":''));
}

// Reject
if (isset($_GET['reject']) && $cid) {
    $id = (int)$_GET['reject'];
    $u  = $conn->prepare("UPDATE applications SET status='rejected_company' WHERE application_id=? AND internship_id IN (SELECT internship_id FROM internships WHERE company_id=?)");
    if ($u) { $u->bind_param('ii',$id,$cid); $u->execute(); }
    setFlash('info','Đã từ chối ứng viên.');
    redirect('company_candidates.php'.($job_filter?"?job=$job_filter":''));
}

// Mark interview passed → bắt đầu thực tập
if (isset($_GET['start_internship']) && $cid) {
    $id  = (int)$_GET['start_internship'];
    $aq  = $conn->prepare("SELECT a.*,sp.student_id,i.company_id,i.internship_id FROM applications a JOIN student_profiles sp ON a.student_id=sp.student_id JOIN internships i ON a.internship_id=i.internship_id WHERE a.application_id=? AND i.company_id=?");
    if ($aq) {
        $aq->bind_param('ii',$id,$cid); $aq->execute();
        $app = $aq->get_result()->fetch_assoc();
        if ($app) {
            $conn->query("UPDATE applications SET status='internship_active' WHERE application_id=$id");
            $ins = $conn->prepare("INSERT IGNORE INTO internship_registrations (student_id,company_id,internship_id,status) VALUES (?,?,?,'active')");
            if ($ins) { $ins->bind_param('iii',$app['student_id'],$cid,$app['internship_id']); $ins->execute(); }
...
if (isset($_GET['pass_interview']) && $cid) {
    ...
    $u  = $conn->prepare("UPDATE applications SET status='interview_passed' WHERE application_id=? AND internship_id IN (SELECT internship_id FROM internships WHERE company_id=?)");
    if ($u) { $u->bind_param('ii',$id,$cid); $u->execute(); }
    // Tạo/cập nhật interview record
    $ins = $conn->prepare("INSERT INTO interviews (application_id,result) VALUES (?,'passed') ON DUPLICATE KEY UPDATE result='passed'");
    if ($ins) { $ins->bind_param('i',$id); $ins->execute(); }
    setFlash('success','🎉 Đánh dấu đậu phỏng vấn! Bấm "Bắt đầu thực tập" để xác nhận.');
    redirect('company_candidates.php'.($job_filter?"?job=$job_filter":''));
}
...
if (isset($_GET['fail_interview']) && $cid) {
    ...
    $u  = $conn->prepare("UPDATE applications SET status='interview_failed' WHERE application_id=? AND internship_id IN (SELECT internship_id FROM internships WHERE company_id=?)");
    if ($u) { $u->bind_param('ii',$id,$cid); $u->execute(); }
    $ins = $conn->prepare("INSERT INTO interviews (application_id,result) VALUES (?,'failed') ON DUPLICATE KEY UPDATE result='failed'");
    if ($ins) { $ins->bind_param('i',$id); $ins->execute(); }
...
if (isset($_GET['delete']) && $cid) {
    $id = (int)$_GET['delete'];
    $u  = $conn->prepare("DELETE FROM applications WHERE application_id=? AND internship_id IN (SELECT internship_id FROM internships WHERE company_id=?) AND status NOT IN ('internship_active','internship_completed')");
    if ($u) { $u->bind_param('ii',$id,$cid); $u->execute(); }
...
$my_jobs = $cid ? safeQuery($conn,"SELECT internship_id,title FROM internships WHERE company_id=$cid ORDER BY created_at DESC") : [];

// Fetch candidates
$where = "i.company_id=$cid";
if ($job_filter) $where .= " AND a.internship_id=$job_filter";
$candidates = safeQuery($conn,"SELECT a.*,sp.full_name,sp.student_code,sp.gpa,sp.major,sp.avatar AS s_av,sp.student_id AS sp_sid,u.email,i.title AS job_title,iv.result AS iv_result,iv.interview_date,ir.registration_id
  FROM applications a
...
foreach(['pending_admin','approved_admin','approved_company','rejected_company','interview_passed','internship_active'] as $s)
    $cnt[$s] = 0;
foreach($candidates as $c) $cnt[$c['status']] = ($cnt[$c['status']]??0) + 1;
?>
...
<?php foreach($candidates as $i=>$a):
  ...
  $can_pass_fail = ($a['status']==='approved_company'); // Công ty đã chấp nhận → chờ phỏng vấn
  ...
  <div class="card h-100" style="border:1.5px solid rgba(164,195,162,.25)">
    <div class="card-body">
      ...
      <div class="mb-2 p-2" style="background:rgba(74,138,150,.08);border-radius:8px;font-size:.8rem">
        ...
        <?php if($a['iv_result']==='passed'): ?><span class="badge ms-1" style="background:rgba(74,158,106,.2);color:#2d6a40">🎉 Đậu</span><?php elseif($a['iv_result']==='failed'): ?><span class="badge ms-1" style="background:rgba(192,96,80,.15);color:#9a3030">Rớt</span><?php endif; ?>
      </div>
      <?php endif; ?>
      ...
      <?php if($a['cv_file']??''): ?>
      <div class="mb-2">
        <a href="<?=UPLOAD_URL.'/'.$a['cv_file']?>" target="_blank" class="btn btn-secondary btn-sm w-100"><i class="bi bi-file-earmark-pdf me-1"></i>Xem CV</a>
      </div>
      <?php endif; ?>
      ...
      <div class="d-flex gap-2 flex-wrap mt-2">
        <?php if($can_approve): ?>
        <a href="?approve=<?=$a['application_id']?><?=$job_filter?"&job=$job_filter":''?>" class="btn btn-success btn-sm flex-fill" onclick="return confirm('Chấp nhận ứng viên này?')">
          <i class="bi bi-person-check me-1"></i>Chấp nhận
        </a>
        <a href="?reject=<?=$a['application_id']?><?=$job_filter?"&job=$job_filter":''?>" class="btn btn-danger btn-sm" onclick="return confirm('Từ chối ứng viên?')">
          <i class="bi bi-x-lg"></i>
        </a>
        <?php endif; ?>

        <?php if($can_pass_fail): ?>
        <a href="?pass_interview=<?=$a['application_id']?><?=$job_filter?"&job=$job_filter":''?>" class="btn btn-success btn-sm flex-fill" onclick="return confirm('Xác nhận sinh viên ĐẬU phỏng vấn?')">
          <i class="bi bi-check-circle-fill me-1"></i>Đậu PV
        </a>
        <a href="?fail_interview=<?=$a['application_id']?><?=$job_filter?"&job=$job_filter":''?>" class="btn btn-danger btn-sm" onclick="return confirm('Xác nhận sinh viên RỚT phỏng vấn?')">
          <i class="bi bi-x-circle-fill me-1"></i>Rớt PV
        </a>
        <?php endif; ?>

        <?php if($can_message): ?>
        <a href="<?=BASE_PATH?>/modules/messages/chat.php?student_id=<?=$a['sp_sid']?>&app_id=<?=$a['application_id']?>" class="btn btn-primary btn-sm <?=!$can_approve?'flex-fill':''?>">
        ...
        <?php if($can_delete): ?>
        <a href="?delete=<?=$a['application_id']?><?=$job_filter?"&job=$job_filter":''?>" class="btn btn-danger btn-sm" onclick="return confirm('Xóa đơn này?')" title="Xóa">
