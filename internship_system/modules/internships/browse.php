...
$sql="SELECT i.*,cp.company_name,cp.logo,cp.address AS c_address,
      (SELECT COUNT(*) FROM applications WHERE internship_id=i.internship_id AND status NOT IN ('rejected_admin','rejected_company','interview_failed')) AS applied_count,
      (SELECT application_id FROM applications WHERE student_id=? AND internship_id=i.internship_id LIMIT 1) AS my_app
      FROM internships i JOIN company_profiles cp ON i.company_id=cp.company_id
      WHERE i.status='open'";
$params=[$sid]; $types='i';
if($search){ $sql.=" AND (i.title LIKE ? OR i.description LIKE ? OR cp.company_name LIKE ?)"; $like="%$search%"; $params[]=$like;$params[]=$like;$params[]=$like; $types.='sss'; }
  ...
  <div class="card h-100" style="border:1.5px solid rgba(164,195,162,.2)">
    <div class="card-body d-flex flex-column">
      ...
      <div class="mt-auto">
        ...
        <?php else: ?>
        <a href="<?=BASE_PATH?>/modules/applications/apply.php?internship_id=<?=$j['internship_id']?>" class="btn btn-primary w-100 btn-sm">
